...
void
jpeg_memory_src(j_decompress_ptr cinfo, const unsigned char *buffer,
				size_t bufsize)
{
...
image_s *
image_new_from_jpeg(const char *path, int is_file, const uint8_t *buf, int size,
					int scale, int rotate)
{
	...
	if (cinfo.output_components == 3)
	{
		...
		for (y = 0; y < h; y += cinfo.rec_outbuf_height)
		{
			...
			{
				rx =
					(rotate & (ROTATE_180 | ROTATE_270)) ? (x - w + 1) * -1 : x;
				ofs = (rotate & (ROTATE_90 | ROTATE_270)) ? ry + (rx * h)
														  : rx + (ry * w);
				if (ofs < maxbuf)
					vimage->buf[ofs] = COL(ptr[x + x + x], ptr[x + x + x + 1],
										   ptr[x + x + x + 2]);
			}
		}
		free(ptr);
	...
	else if (cinfo.output_components == 1)
	{
		...
		for (y = 0; y < h; y += cinfo.rec_outbuf_height)
		{
			...
			for (i = 0; i < cinfo.rec_outbuf_height; i++)
			{
				for (x = 0; x < w; x++)
				{
					rx = (rotate & (ROTATE_180 | ROTATE_270)) ? (x - w + 1) * -1
															  : x;
					ofs = (rotate & (ROTATE_90 | ROTATE_270)) ? ry + (rx * h)
															  : rx + (ry * w);
					if (ofs < maxbuf)
