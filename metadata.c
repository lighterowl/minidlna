...
check_for_captions(const char *path, int64_t detailID)
{
	...
	if (!detailID)
	{
		...
		if (detailID <= 0)
		{
			// DPRINTF(E_MAXDEBUG, L_METADATA, "No file found for caption
			// %s.\n", path);
			return;
		}
	}
...
int64_t
GetFolderMetadata(const char *name, const char *path, const char *artist,
...
GetAudioMetadata(const char *path, const char *name)
{
	...
	ret = sql_exec(db,
				   "INSERT into DETAILS"
				   " (PATH, SIZE, TIMESTAMP, DURATION, CHANNELS, BITRATE, "
				   "SAMPLERATE, DATE,"
				   "  TITLE, CREATOR, ARTIST, ALBUM, GENRE, COMMENT, DISC, "
				   "TRACK, DLNA_PN, MIME, ALBUM_ART) "
				   "VALUES"
				   " (%Q, %lld, %lld, '%s', %d, %d, %d, %Q, %Q, %Q, %Q, %Q, "
				   "%Q, %Q, %d, %d, %Q, '%s', %lld);",
				   path, (long long)file.st_size, (long long)file.st_mtime,
				   m.duration, song.channels, song.bitrate, song.samplerate,
				   m.date, m.title, m.creator, m.artist, m.album, m.genre,
				   m.comment, song.disc, song.track, m.dlna_pn,
...
GetImageMetadata(const char *path, const char *name)
{
	...
	if (e)
	{
		...
		if (e)
		{
			strncpyt(model, exif_entry_get_value(e, b, sizeof(b)),
					 sizeof(model));
	...
	if (ed->size)
	{
		...
		if (ed->size > 12000)
		{
			imsrc = image_new_from_jpeg(NULL, 0, ed->data, ed->size, 1,
										ROTATE_NONE);
			if (imsrc)
			{
...
GetVideoMetadata(const char *path, const char *name)
{
	...
	if (astream)
	{
		...
		case AV_CODEC_ID_AAC:
			...
			case AAC_LC_ER:
				...
				{
					DPRINTF(E_DEBUG, L_METADATA,
							"Unsupported AAC: sample rate is not 8000 < %d < "
							"48000\n",
	...
	if (vstream)
	{
		...
		case AV_CODEC_ID_MPEG2VIDEO:
			...
			if (strcmp(ctx->iformat->name, "mpegts") == 0)
			{
				int raw_packet_size;
				int dlna_ts_present =
					dlna_timestamp_is_present(path, &raw_packet_size);
				DPRINTF(E_DEBUG, L_METADATA,
			...
			if (strcmp(ctx->iformat->name, "mpegts") == 0)
			{
				...
				if (lav_sample_aspect_ratio(vstream).num)
				{
					av_reduce(&display_aspect_ratio.num,
							  ...
							  lav_width(vstream) *
								  lav_sample_aspect_ratio(vstream).num,
							  lav_height(vstream) *
								  lav_sample_aspect_ratio(vstream).den,
							  1024 * 1024);
				}
				...
				case FF_PROFILE_H264_CONSTRAINED_BASELINE:
					...
					/* Fall back to Main Profile if it doesn't match a Baseline
					 * DLNA profile. */
					else
						off -= 3;
				default:
				case FF_PROFILE_H264_MAIN:
					...
					{
						DPRINTF(E_DEBUG, L_METADATA,
								"Unsupported h.264 video profile! [%s, %dx%d, "
								"%lldbps : %s]\n",
								m.dlna_pn, lav_width(vstream),
								lav_height(vstream),
				...
				case FF_PROFILE_H264_HIGH:
					...
					{
						DPRINTF(E_DEBUG, L_METADATA,
								"Unsupported h.264 HP video profile! [%lldbps, "
								"%d audio : %s]\n",
								(long long)lav_bit_rate(vstream), audio_profile,
								basepath);
						free(m.dlna_pn);
						m.dlna_pn = NULL;
					}
		...
		case AV_CODEC_ID_MPEG4:
			...
			DPRINTF(E_DEBUG, L_METADATA,
					...
					basepath, isprint(fourcc[0]) ? fourcc[0] : '_',
			...
			if (strcmp(ctx->iformat->name, "mov,mp4,m4a,3gp,3g2,mj2") == 0)
			{
				...
				if (ends_with(path, ".3gp"))
				{
					...
					default:
						DPRINTF(E_DEBUG, L_METADATA,
								"No DLNA profile found for MPEG4-P2 3GP/%d "
								"file %s\n",
				...
				else
				{
					...
					{
						DPRINTF(E_DEBUG, L_METADATA,
								"Unsupported h.264 video profile! [%dx%d, "
								"%lldbps]\n",
		...
		case AV_CODEC_ID_WMV3:
			/* I'm not 100% sure this is correct, but it works on everything I
	...
	if (strcmp(ctx->iformat->name, "asf") == 0)
	{
		if (readtags((char *)path, &video, &file, "en_US", "asf") == 0)
		{
			...
			{
				m.artist =
					escape_tag(trim(video.contributor[ROLE_TRACKARTIST]), 1);
			}
			if (video.contributor[ROLE_ALBUMARTIST] &&
				*video.contributor[ROLE_ALBUMARTIST])
			{
				m.creator =
					escape_tag(trim(video.contributor[ROLE_ALBUMARTIST]), 1);
			}
			else
			{
				m.creator = m.artist;
...
video_no_dlna:
	...
	ret = sql_exec(db,
				   "INSERT into DETAILS"
				   " (PATH, SIZE, TIMESTAMP, DURATION, DATE, CHANNELS, "
				   "BITRATE, SAMPLERATE, RESOLUTION,"
				   "  TITLE, CREATOR, ARTIST, GENRE, COMMENT, DLNA_PN, MIME, "
				   "ALBUM_ART, DISC, TRACK) "
				   "VALUES"
				   " (%Q, %lld, %lld, %Q, %Q, %u, %u, %u, %Q, '%q', %Q, %Q, "
				   "%Q, %Q, %Q, '%q', %lld, %u, %u);",
				   path, (long long)file.st_size, (long long)file.st_mtime,
				   m.duration, m.date, m.channels, m.bitrate, m.frequency,
				   m.resolution, m.title, m.creator, m.artist, m.genre,
				   m.comment, m.dlna_pn, m.mime, album_art, m.disc, m.track);
	if (ret != SQLITE_OK)
	{
		DPRINTF(E_ERROR, L_METADATA, "Error inserting details for '%s'!\n",
				path);
