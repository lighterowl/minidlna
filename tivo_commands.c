...
callback(void *args, int argc, char **argv, char **azColName)
{
	...
	if (strncmp(class, "item", 4) == 0)
	{
		...
		if (artist)
		{
			strcatf(str, "<ArtistName>%s</ArtistName>",
					tivo_unescape_tag(artist));
		}
		if (album)
		{
			strcatf(str, "<AlbumTitle>%s</AlbumTitle>",
					tivo_unescape_tag(album));
		}
		if (genre)
		{
			strcatf(str, "<MusicGenre>%s</MusicGenre>",
					tivo_unescape_tag(genre));
		}
		if (resolution)
		{
		...
		if (sampleFrequency)
		{
			strcatf(str, "<SourceSampleRate>%s</SourceSampleRate>",
					sampleFrequency);
		}
		strcatf(str,
				"</Details><Links>"
...
static void
SendContainer(struct upnphttp *h, const char *objectID, int itemStart,
			  int itemCount, char *anchorItem, int anchorOffset, int recurse,
			  char *sortOrder, char *filter, unsigned long int randomSeed)
{
	...
	else
	{
		strcpy(myfilter, "MIME in ('image/jpeg', 'audio/mpeg', 'video/mpeg', "
						 "'video/x-tivo-mpeg', 'video/x-tivo-mpeg-ts') or "
	...
	if (anchorItem)
	{
		...
		sql = sqlite3_mprintf("SELECT %s from OBJECTS o left join DETAILS d on "
							  "(o.DETAIL_ID = d.ID)"
							  " where %s and (%s)"
							  " %s"
							  " order by %s",
		...
		if (strcmp(command, "QueryContainer") == 0)
		{
			...
			else
			{
				SendContainer(h, container, itemStart, itemCount, anchorItem,
							  anchorOffset, recurse, sortOrder, filter,
							  randomSeed);
			}
		}
