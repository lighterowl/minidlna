...
_get_aactags(char *file, struct song_metadata *psong)
{
	...
	if (atom_offset != -1)
	{
		...
		{
			if (fread((void *)&current_size, 1, sizeof(int), fin) !=
				sizeof(int))
				break;
			...
			if (current_size <= 7 ||
				current_size > 1 << 24) // something not right
				break;
			...
			if (fread(current_data, 1, current_size - 8, fin) !=
				current_size - 8)
				break;
			...
			if (!memcmp(current_atom,
				...
				psong->contributor[ROLE_CONDUCTOR] =
					strdup((char *)&current_data[16]);
							 ...
							 "wrt",
							 4))
				psong->contributor[ROLE_COMPOSER] =
					strdup((char *)&current_data[16]);
			else if (!memcmp(current_atom,
							 "\xA9"
							 "grp",
			...
			else if (!memcmp(current_atom, "trkn", 4))
			{
				...
				psong->total_tracks =
					(current_data[20] << 8) | current_data[21];
			}
	...
	if (atom_offset != -1)
	{
		...
		if (atom_offset != -1)
		{
			...
			if (!fread((void *)&buffer, 1, 1, infile) || (buffer[0] != 0x04) ||
				...
				goto bad_esds;
			fseek(infile, 10,
				  SEEK_CUR); // 10 bytes into section 4 should be average
			...
			if (fread((void *)&buffer, 2, 1, infile))
			{
				profile_id =
					(buffer[0] >>
					 3); // first 5 bits of setup data is the Audo Profile ID
				/* Frequency index: (((buffer[0] & 0x7) << 1) | (buffer[1] >>
				 * 7))) */
...
bad_esds:
	...
	atom_offset =
		_aac_lookforatom(infile, "mdat", (unsigned int *)&atom_length);
