...
_get_mp3tags(char *file, struct song_metadata *psong)
{
	...
	id3_file_close(pid3file);
	// DEBUG DPRINTF(E_INFO, L_SCANNER, "Got id3 tag successfully for
	// file=%s\n", file);
	return 0;
}
...
static void
_mp3_get_average_bitrate(FILE *infile, struct mp3_frameinfo *pfi,
						 const char *fname)
{
	off_t file_size;
...
_get_mp3fileinfo(char *file, struct song_metadata *psong)
{
	...
	while (!found)
	{
		...
		while (!found)
		{
			...
			{
				if (!strncasecmp((char *)&buffer[index + fi.xing_offset + 4],
								 "XING", 4))
				{
					/* no need to check further... if there is a xing header
					 * there, this is definately a valid frame */
					found = 1;
					fp_size += index;
				}
				else
				{
					/* No Xing... check for next frame to validate current fram
					 * is correct */
					fseek(infile, fp_size + index + fi.frame_length, SEEK_SET);
					if (fread(frame_buffer, 1, sizeof(frame_buffer), infile) ==
						sizeof(frame_buffer))
					{
						if (!_decode_mp3_frame((unsigned char *)frame_buffer,
											   &fi))
						{
							found = 1;
							fp_size += index;
					...
					if (!found)
					{
						// cannot find second frame. Song may be too short. So
						// assume first frame is valid.
						found = 1;
						fp_size += index;
					}
				}
	...
	if (!psong->song_length)
	{
		...
		{
			psong->song_length = (int)((double)(fi.number_of_frames *
												fi.samples_per_frame * 1000.) /
