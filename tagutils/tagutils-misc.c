...
#ifdef HAVE_ICONV
static iconv_result
do_iconv(const char *to_ces, const char *from_ces, ICONV_CONST char *inbuf,
		 size_t inbytesleft, char *outbuf_orig, size_t outbytesleft_orig)
{
	size_t rc;
...
#else  // HAVE_ICONV
static iconv_result
do_iconv(const char *to_ces, const char *from_ces, char *inbuf,
		 size_t inbytesleft, char *outbuf_orig, size_t outbytesleft_orig)
{
	return ICONV_FATAL;
}
...
_get_utf8_text(const id3_ucs4_t *native_text)
{
	...
	{
		// (2) try default -> utf8
		rc = do_iconv("UTF-8", iconv_map[i].cpnames[0], in, strlen(in),
...
vc_scan(struct song_metadata *psong, const char *comment, const size_t length)
{
	...
	// COMPOSER, ARRANGER, LYRICIST, AUTHOR, CONDUCTOR, PERFORMER, ENSEMBLE,
	// PART PARTNUMBER, GENRE, DATE, LOCATION, COMMENT
