...
static int
_asf_read_extended_stream_object(FILE *fp, struct song_metadata *psong,
								 uint32_t size)
{
	int i, len;
	long off;
...
_get_asffileinfo(char *file, struct song_metadata *psong)
{
	...
	while (NumObjects > 0)
	{
		...
		if (IsEqualGUID(&tmp.ID, &ASF_FileProperties))
		{
			...
			psong->song_length =
				le64_to_cpu(FileProperties.PlayDuration) / 10000;
		...
		else if (IsEqualGUID(&tmp.ID, &ASF_ContentDescription))
		{
			...
			if (_asf_load_string(fp, ASF_VT_UNICODE, TitleLength, buf,
								 sizeof(buf)))
			{
				if (buf[0])
		...
		else if (IsEqualGUID(&tmp.ID, &ASF_ExtendedContentDescription))
		{
			...
			while (NumEntries > 0)
			{
				...
				_asf_load_string(fp, ASF_VT_UNICODE, NameLength, buf,
								 sizeof(buf));
				...
				{
					if (_asf_load_string(fp, ValueType, ValueLength, buf,
										 sizeof(buf)))
						if (buf[0])
				...
				else if (!strcasecmp(buf, "Genre") ||
						 !strcasecmp(buf, "WM/Genre"))
				{
					if (_asf_load_string(fp, ValueType, ValueLength, buf,
										 sizeof(buf)))
						if (buf[0])
				...
				else if (!strcasecmp(buf, "Year") ||
						 !strcasecmp(buf, "WM/Year"))
				{
					if (_asf_load_string(fp, ValueType, ValueLength, buf,
										 sizeof(buf)))
						if (buf[0])
