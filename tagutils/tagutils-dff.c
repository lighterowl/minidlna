...
_get_dfffileinfo(char *file, struct song_metadata *psong)
{
	...
	while (totalcount < totalsize - 4)
	{
		...
		{
			// DPRINTF(E_WARN, L_SCANNER, "Could not read chunk header from
		...
		if (strncmp((char *)ckbuf, "PROP", 4) == 0)
		{
			...
			if (!(rt = fread(propckData, propckDataSize, 1, fp)))
			{
				DPRINTF(E_WARN, L_SCANNER,
						"Could not read Property chunk from %s\n", file);
				fclose(fp);
				return -1;
			}
			...
			while (count < propckDataSize)
			{
				...
				else if (strncmp((char *)propckData + count, "CMPR", 4) == 0)
				{
					...
					count += 8;
					strncpy((char *)compressionType, (char *)propckData + count,
							4);
			...
			{
				DPRINTF(E_WARN, L_SCANNER,
						"Invalid DSD/DST Sound Data chunk in %s\n", file);
				fclose(fp);
				return -1;
			}
		...
		else if (!strncmp((char *)ckbuf, "DIIN", 4))
		{
			...
			while (icount < diinckDataSize)
			{
				...
				else if (!strncmp((char *)diinckData + icount, "DIAR", 4))
				{
					...
					strncpy((char *)arttext, (char *)diinckData + icount,
							sizeof(arttext) - 1);
					psong->contributor[ROLE_ARTIST] =
						strdup((char *)&arttext[0]);
				...
				else if (!strncmp((char *)diinckData + icount, "DITI", 4))
				{
					...
					strncpy((char *)titletext, (char *)diinckData + icount,
							sizeof(titletext) - 1);
	...
	// int)totalsize); DPRINTF(E_DEBUG, L_SCANNER, "propckDataSize is
	// 0x%016lx\n", (long unsigned int)propckDataSize); DPRINTF(E_DEBUG,
	// L_SCANNER, "cmprckDataSize is 0x%016lx\n", (long unsigned
	// int)cmprckDataSize); DPRINTF(E_DEBUG, L_SCANNER, "abssckDataSize is
	// 0x%016lx\n", (long unsigned int)abssckDataSize); DPRINTF(E_DEBUG,
	// L_SCANNER, "lscockDataSize is 0x%016lx\n", (long unsigned
	// int)lscockDataSize); DPRINTF(E_DEBUG, L_SCANNER, "dsdsdckDataSize is
	// 0x%016lx\n", (long unsigned int)dsdsdckDataSize); DPRINTF(E_DEBUG,
	// L_SCANNER, "dstickDataSize is 0x%016lx\n", (long unsigned
	// int)dstickDataSize); DPRINTF(E_DEBUG, L_SCANNER, "comtckDataSize is
	// 0x%016lx\n", (long unsigned int)comtckDataSize); DPRINTF(E_DEBUG,
	// L_SCANNER, "diinckDataSize is 0x%016lx\n", (long unsigned
	// int)diinckDataSize); DPRINTF(E_DEBUG, L_SCANNER, "diarckDataSize is
	// 0x%016lx\n", (long unsigned int)diarckDataSize); DPRINTF(E_DEBUG,
	// L_SCANNER, "ditickDataSize is 0x%016lx\n", (long unsigned
	// int)ditickDataSize); DPRINTF(E_DEBUG, L_SCANNER, "manfckDataSize is
