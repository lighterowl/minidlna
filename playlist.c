...
fill_playlists(void)
{
	...
	for (i = 3; i < rows * 3; i++)
	{
		...
		while (next_plist_track(&plist, &file, NULL, type) == 0)
		{
			...
			{
				// DEBUG DPRINTF(E_DEBUG, L_SCANNER, "%d: already in
				// database\n", plist.track);
				found++;
				freetags(&plist);
				continue;
			...
			if (last_dir)
			{
				if (hash == last_hash)
				{
					...
					detailID = sql_get_int_field(
						db, "SELECT ID from DETAILS where PATH = '%q/%q'",
						last_dir, fname);
				}
				else
					detailID = -1;
		...
		retry:
			...
			found:
				...
				sql_exec(
					...
					"SELECT"
					" '%s$%llX$%d', '%s$%llX', CLASS, DETAIL_ID, NAME, "
					"OBJECT_ID from OBJECTS"
					" where DETAIL_ID = %lld and OBJECT_ID glob '" BROWSEDIR_ID
					"$*'",
					MUSIC_PLIST_ID, plID, plist.track, MUSIC_PLIST_ID, plID,
					detailID);
				if (!last_dir)
				{
					last_dir = sql_get_text_field(
						db, "SELECT PATH from DETAILS where ID = %lld",
						detailID);
					if (last_dir)
					{
